...
> A Token-2022 mint can take a holder's funds three ways a plain decode misses: a transfer hook swapped for a sell-blocker after audit, a live permanent delegate that seizes balances, and a near-100% honeypot fee. This skill catches all three, follows an active hook to its program to flag upgradeable bytecode, and proves every verdict offline (129 checks, 11 of 11 real mainnet mints).
...
Read-only and offline by design (no signing, no sending; the live mainnet check is a single gated CI step). The second hop reports a hook program's mutability and names its upgrade authority, not a full bytecode audit. It scores capability, not intent, so a regulated issuer like PYUSD reads CRITICAL too. A Token-2022 specialist: it delegates core program development to `solana-dev`.
...
```bash
cd examples && make verify   # 129 checks: TypeScript mint, Rust hook, mint inspector (npm run evals runs the scored suite)
```
